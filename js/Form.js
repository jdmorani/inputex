(function() {
  ...
  lang.extend(inputEx.Form, inputEx.Group, {
    ...
    setOptions: function(options) {
      ...
      this.options.autocomplete = lang.isUndefined(options.autocomplete) ? inputEx.browserAutocomplete : (options.autocomplete === false || options.autocomplete === "off") ? false : true;
    ...
    renderButtons: function() {
      ...
      this.buttonDiv = inputEx.cn('div', {
        className: 'inputEx-Form-buttonBar'
      });

      for (var i = 0; i < buttonsNb; i++) {
      ...
      this.buttonDiv.appendChild(inputEx.cn('div', null, {
        clear: 'both'
      }));
    ...
    /**
     * Called when one of the form subfields is updated.
    ...
    initEvents: function() {
      ...
      for (var i = 0, length = this.buttons.length; i < length; i++) {
        ...
        this.buttons[i].submitEvent.subscribe(function() {
          this.submitEvent.fire();
        }, this, true);
    ...
    renderMask: function() {
      ...
      if (YAHOO.env.ua.ie) {
        Dom.setStyle(this.divEl, "zoom", 1);
      }

      // Render mask over the divEl
      this.formMask = inputEx.cn('div', {
        className: 'inputEx-Form-Mask'
      }, {
        display: 'none',
   ...
   */
    toggleSelectsInIE: function(show) {
      ...
      if ( !! YAHOO.env.ua.ie && YAHOO.env.ua.ie < 7) {
        ...
        function() {
          return true;
        }, "select", this.divEl, function(el) {
          method.call(that, el, "inputEx-hidden");
        });
      }
    },
